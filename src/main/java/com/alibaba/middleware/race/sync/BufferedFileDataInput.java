...
public class BufferedFileDataInput {
    private static Logger logger = LoggerFactory.getLogger(BufferedFileDataInput.class);
    // Read parameters.
    private File file;
    private int size;
    ...
    public long waitAvailable(int requested, int waitMillis) throws IOException, InterruptedException {
        ...
        while (available() < requested && System.currentTimeMillis() < timeoutMillis) {
            ...
            if (System.currentTimeMillis() > nextReportMillis) {
                if (logger.isDebugEnabled()) logger.debug("Waited 1000ms for input to appear");
